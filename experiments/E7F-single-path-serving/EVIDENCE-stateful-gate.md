...
- 日期：2026-07-22 · 状态：**证据（本文）→ 人已裁定放行**。
- **本文是喂给决定的证据。裁决由人作出**：**放行 E7F stateful serving 路径、默认开**
  （2026-07-22）。裁决**不是**移常数（分数不打平 610≠614，重标或自我基线都不成立），
  而是一个**非逐位路径的门定义**（判据 1 = "无统计显著回归"，判据 2 包络不变、按
  "零 stateful-引入越界"评估）。**门定义与放行理由的权威记录在 TARGET §1.3
  "非逐位路径门定义"**；本文只是其四条证据的明细。
  ⚠️ 硬边界未动：任何 stateful-引入错误不放宽容差；只覆盖非逐位路径求和序-显著性量级；
  未来显著回归或任一 stateful-引入越界仍硬 FAIL。
...
（§9.5 ≥3 轮**已核对**：indep / indep2 / indep3 三轮 predicted_tokens 逐字节相同、
**spread 0**（eager 892 / stateful 900 / perturbed 894，三轮不变），
artifact `evidence-indep{,2,3}`。）
