...
## 追加:attention kernel 杠杆判活(6.49×)

既然 prefill 58% 在 attention,直接量化"接入 reference tilelang `sparse_attn`"
的收益(`runtime/c2f_attention_kernel_probe.py`,titan065 单卡;两者签名完全
一致,可直接对拍;sm89 限制下 tilelang 臂用 4×16 head-loop,torch 臂按 runtime
实际行为做 1024 行分块):

| 口径(seqlen 8192,640 候选,64 heads,d=512) | 时间 | 峰值显存 |
|---|---:|---:|
| runtime torch masked-einsum(行分块 1024) | **106.07 ms** | 3.50 GiB |
| reference tilelang sparse_attn(4×h16) | **16.36 ms** | 1.59 GiB |
| **加速** | **6.49×** | −55% |

数值(seqlen 512 全 64 heads):rel_fro **1.93e-3**、max_abs 1.22e-4——bf16 级
一致(torch 臂内部走 fp32,tilelang 走 bf16+fp32 累加),量级与既有 attention
门(限 0.04)相容,接入时按冻结质量门正式过 oracle。

**折算到 pass**:ratio-4 层候选 640(= window 128 + topk 512),稀疏核占其
164 ms/层中的 ~106 ms → 每层省 ~90 ms × 5 层 = **449 ms**;ratio-128 层候选
192,按比例省 ~27 ms/层 × 6 层 = **162 ms**。合计 **~611 ms / 1.981 s** →
pass ~1.37 s → **~24k input tok/s(+45%)**;再叠 prefill HC 融合(0.313 s)
可近 27k。§5.3 的 30–40k 带在此路径上可及。

**单池投影**:P=24k 时 T = 1/(1/8733 + 8/24000) = **2.23k**;P=30k → 2.62k;
P=40k → 3.18k ≈ 带下沿。即**单池 3.2–4.2k 需要 prefill 与 decode 两侧同时
接近各自上限**,与 decode 侧的结论一致。

Artifacts:`c2f-attn-kernel.json`;探针 `runtime/c2f_attention_kernel_probe.py`。
