...
不是实测**——直接实测值是 §4.1 的 11,424 → 14,630。要拿到直接可比的 16.6k → 24k
实测对,需要先解掉 MoE 的分配器缺陷(该缺陷本身价值 +32% 吞吐,重归因已列为待办)。
...
4. **e0wf/e0ef 的 `sparse_control` 冻结限 3e-3 是本次最紧的门**(用掉 57–60%)。
   其余各级都有 ≥1.7× 到 3× 余量。
5. **MoE 双模缺陷今天落在慢分支**(§4.3),且与 `max_memory_reserved` 是否撑到接近
   整卡高度相关(20.50 GiB vs 23.98 GiB)。这是重归因里"prefill MoE 的 fp32 临时量
   应预分配复用"那条待办的又一次复现,现在它是 prefill 的头号瓶颈。
