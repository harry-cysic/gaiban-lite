...
## 640 位重测(原定目标,现已可做):两臂**完全打平**

`--prefill-chunk 4096` 解除了 8192 档的 MoE 缓冲限制(见 D0L README 的更正),
于是最初想做的"以 640 位新参照重测 E6F"终于可做。两臂同代码、同 oracle、
同 chunk、显存起点均实测 1 MiB。
...
**净差 0**。3 条更差、3 条更好、4 条持平,符号检验 **p = 1.000**。

### 内部结构才是关键

| 子集 | 基线 | 分片 |
|---|---|---|
| 前 8 条(= 冻结 v1 集) | 494/512 | **491/512** |
| 8192 档(此前从未判过) | 120/128 | **123/128** |

两件事同时成立:

1. **−3 在冻结子集上被逐字复现**(494 / 491)。门是确定性的(D0L 已记录复跑
   逐 prompt 完全一致),所以 −3 **不是测量噪声**,是那 8 条 prompt 上真实且
   可复现的差异。
2. **该差异不外推**:多出来的 8192 档把 3 位**还了回来**(+3),总账归零。

即:**−3/512 是抽样噪声(sampling noise),不是测量噪声(measurement noise)**。
它稳定地存在于那 8 条上,但不是模型级的系统性回归。这两个词不可互换。

### 与独立集相互印证

| 证据 | 位数 | 净差 | 方向 | p |
|---|---|---|---|---|
| D0L-indep(独立重抽) | 927 | −1 | 7 差 / 6 好 | 1.000 |
| D0L v2(640,含 8192) | 640 | **0** | 3 差 / 3 好 | 1.000 |

两条独立证据、不同 prompt 集、都是双向翻转、都不显著。

**顺带验证了 chunk 的数值中性**:分块后前 8 条两臂分别是 494 / 491,与历史
**未分块**的 494 / 491 逐字相同——chunk 没有改变任何一臂的判定。

### 状态未变:仍不满足 §1.3

§1.3 定义在**冻结 512 位集**上(≥494),E6F 在该集上仍是 **491,仍 FAIL**,
且"容差在任何情况下不得放宽"。本节加强的是"分片不引入系统性质量回归"这一
命题的证据,**不是** "E6F 过了 §1.3"。是否据此重新标定门基准,仍是**需要人
来做的决定**,本竖条不自行更改。

---

## 放行（2026-07-22）：门基准迁移到 640 位后重判，两条判据 PASS

人已决定把 §1.3 的门基准从冻结 512 位迁到 **D0L v2（640 位）**，两个常数由
**该集的未分片基线**重新标定。据此重判 E6F：

| 判据 | E6F | 基线 / 包络 | 结果 |
|---|---|---|---|
| 1 分数不降 | **614/640** | 614/640 | **PASS**（打平） |
| 2 `top2_gap` 不越包络 | **0.955883026123** | 0.959503173828 | **PASS**（严格在内） |

**基线确定性已复跑验证**：`v2chunk` 与 `v2chunkr2` **逐 prompt 预测逐字相同**、
同为 614/640、包络逐位相同——与旧门当年 `e2e-long-base-r2` 的证据等级一致。

### 为什么这是重新标定而不是放宽

- **判据 2 的常数逐位未变**（0.959503173828125 两集相同：取到包络的失配落在
  v1 子集内，新增两条 8192 都没超过它）。E6F 又落在它**以内**——
  判据 2 是在**更严的证据**下通过的。
- **判据 1 的规则未变**（"不低于同一集上的未分片基线"）。变的是集，
  614 是**基线自己跑出来的**，不是为 E6F 选的阈值。

### 必须同时看见的另一面

**E6F 本身没有变——在 v1 子集上它仍是 491 vs 494，且可复现。**
是**门迁移**让它通过的。迁移的依据是那 −3 **不外推**
（8192 档还回 +3；独立 927 位 7 差/6 好，p=1.000），
以及旧常数**不可移植**（独立集上未分片基线自己就 1.0178 > 0.9595）。

### 已翻默认

`build_physical_stage(attention_tp_shard=True)`；各测量入口改为
`--attention-tp-shard / --no-attention-tp-shard`（默认开）。

**实测确认默认生效**（不靠假设）：不带任何 flag 跑门，
**预测 token 与分片臂逐字相同、与未分片臂不同**，stage 0 载入后 free
**5.65 GiB**（分片口径；未分片是 4.23）。

⚠️ **口径断层**：**此前所有"不带 flag"的数字都是未分片的**（E1F 闭环
29.2 tok/s 即是）。翻默认后同一条命令给出**分片**结果，**两者不可直接比较**。
⚠️ 该验证同时暴露门的 witness **没有记录 `attention_tp_shard`**（它不符合
`*_mode` 命名，`mode_witness` 抓不到）——已补记 `attention_tp_shard` 与 `argv`。
