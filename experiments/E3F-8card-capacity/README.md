# E3F — 8 卡 TP4×PP2 容量判决

第二十九竖条。**结论：8 卡 TP4×PP2 在当前驻留形态下装不下 43 层模型。**
在 B=1、max_seq 3328（可能的最省配置）下，一个 22 层 stage 在加载第 19 层时
OOM。实测每卡最多 **18 层**（19 层为极限且无余量），PP2 需要 **22 层**。

**因此：M7（方案 A，2× 独立单机、10–16K 聚合）不成立**——该档位所需的 KV 池
远超任何可能的余量。

> **⚠️ 本文初稿的结论超出了证据边界，已于 2026-07-21 收敛。**
> 初稿写的是"8 卡装不下 43 层""M4 的家也不在 8 卡"。但**本文第 4 节自己的
> 字节账就是反例**：attention TP4 分片后 23.02 GiB（余 0.50），再加 FP8 常驻
> 22.39（余 1.13）——**模型装得下**。装不下的是**吞吐档位的大 KV 池**。
> 这个区别不是措辞问题：按 TARGET 的规矩"被证伪的条目移入 §5 而不是删除"，
> 一旦以"根本装不下"的形式进入 §5，8 卡就成了关闭问题、后续不会回头重问；
> 而实际上**它是一个还没被正确提问过的问题**（见 §5 与 TARGET §7.7）。

---

## 1. 为什么这个测试是直接的、不是外推

TARGET §7.2 把"8 卡形态验证"列为性价比最高的一次验证，因为它同时卡住
M4 家园、M7 与全部单机口径计划值。

8 卡 TP4×PP2 的**一个 stage 就是 4 张卡上的一个 TP4 super-stage**，
层数 21–22。E2F 的探针（`e2f_decode_phase_probe.py`）建的正是这个对象，
只是层数可配。所以 `--layers 0-21` **就是**目标形态的 stage 0，
不是近似、不是外推、无需新代码。

## 2. 实测

titan064 GPU0-3，B=1，max_seq_len 3328，bf16 KV，fused HC：

| 层数 | 结果 | after_load 占用 | 余量 |
|---|---|---:|---:|
| 11（= 16 卡 PP4 的 stage） | ✅ 跑通 | 13.86 GiB | 9.66 GiB |
| 18 | ✅ 跑通（含捕图与 replay） | 22.12 GiB | **1.40 GiB** |
| **22（= 8 卡 PP2 的 stage）** | ❌ **OOM 在第 19 层** | 23.12 GiB 时耗尽 | — |

OOM 原文：`Tried to allocate 512.00 MiB. GPU 0 has a total capacity of
23.52 GiB of which 403.94 MiB is free`，四个 rank 同时。

**边际成本（11→18 层实测）：1.180 GiB/层。**
22 层外推 = 22.12 + 4×1.180 = **26.84 GiB**，超卡容量 3.3 GiB——与实测
OOM 位置一致（第 19 层时已用 23.12 GiB）。

顺带的性能标度（同批运行）：

| 层数 | normal replay p50 | 每层边际 |
|---|---:|---:|
| 11 | 7.652 ms | — |
| 18 | 12.573 ms | **0.7031 ms/层** |

与 E2F 的 0.696 ms/层一致（E2F §3.1），**replay 时间对层数是线性的**。

## 3. 容量为什么不够：字节账

用 E2F §3.5 的实测驻留字节，每 TP rank 每层：

| 项 | 每层每卡 | 22 层 | 占卡容量 |
|---|---:|---:|---:|
| routed experts（FP4，已按 TP4 分片） | 0.797 GiB | **17.53 GiB** | 75% |
| shared expert + gate | 0.020 GiB | 0.43 GiB | 2% |
| attention（**BF16/FP32，每 rank 完整副本**） | 0.232 GiB（均） | **5.09 GiB** | 22% |
| **权重小计** | | **23.05 GiB** | **98%** |
| 非权重（KV + 图池 + workspace） | 0.131 GiB | 2.88 GiB | 12% |
| CUDA context 等固定项 | — | 0.91 GiB | 4% |
| **合计** | | **26.84 GiB** | **114%** |

非权重两项由 11 层（2.35 GiB）与 18 层（3.27 GiB）两个实测点解出：
每层 0.131 GiB + 固定 0.91 GiB。合计 26.84 GiB 与第 2 节的外推一致，
也与"第 19 层 OOM"一致。

**光是权重就要 23.05 GiB，而卡只有 23.52 GiB。**所以装不下与 KV 大小、
batch、图池都无关——是权重的硬容量问题，且 KV 还要另外占 2.88 GiB。

对照 16 卡 PP4（11 层/卡）：权重 11.51 GiB，占 49%，余量充足。这也修正了
TARGET §3 的"权重/卡：16 卡 ~9.4 GiB；8 卡 ~20 GiB"——**实测是 11.51 GiB
与 23.05 GiB**。差额正是 E2F 发现的 attention BF16 常驻税（checkpoint 里
非专家权重是 FP8，runtime 里是 BF16，compressor 还是 FP32）。

## 4. 什么能改变结论（推导，非实测）

以第 3 节的实测字节为输入：

| 形态改动 | 权重/卡（22 层） | + 非权重 3.79 GiB | 余量 | 判定 |
|---|---:|---:|---:|---|
| 当前 | 23.05 GiB | 26.84 | −3.32 | ❌ 超 3.3 GiB |
| attention **按 head 分片** | 20.00 | 23.79 | **−0.27** | ❌ **仍不够** |
| head + column-parallel | 19.28 | 23.07 | **+0.45** | ⚠️ 极限 |
| 再加 FP8 常驻 | ~18.6 | ~22.4 | ~+1.1 | ⚠️ 勉强 |

> **⚠️ 本表已于 2026-07-21 修正。**初稿写的是"attention TP4 分片 → 19.23 GiB、
> 余 +0.50"，隐含 attention 权重能干净地 ÷4。**实际不能**：读前向后发现
> FP32 的 compressor（33.6 MB/层）与 q_lora 下投影**不是 head-parallel**，
> 按 head 自然分片只有 **2.49×**，8 卡仍差 0.27 GiB。要拿到 ~4× 必须再做
> column-parallel，代价是每层多 3–4 次集合通信。推导见
> [`docs/design-attention-tp4-sharding.md`](../../docs/design-attention-tp4-sharding.md)。

**注意这三行的大头都是 routed experts 的 17.53 GiB——它已是 FP4 且已按
TP4 分片，没有进一步压缩余地。**所以 8 卡形态的可行性完全取决于 attention
侧的两个杠杆，而这**正是 E2F 为 M4 识别出的同两个杠杆**。

但即便两个杠杆都做满，余量也只有 **~1.1 GiB**，而且这 3.79 GiB 的非权重
基数是在 **B=1、max_seq 3328** 下测的——KV 随 B 与序列长度线性增长，
吞吐模式要的 KV 池是几个 GiB 量级。**结论：两个杠杆能让 8 卡"装下模型"，
但装不下一个有意义的 KV 池。8 卡形态只可能支撑 B 极小的延迟模式，
不可能支撑 M7 所要的 10–16K 吞吐。**

## 5. 对目标的影响（**范围已收敛**）

**成立的证伪**（实测支撑）：

1. **M7（方案 A：2× 独立单机 + 前置路由，计划 decode 10–16K）不成立**——
   它要求整模型 + 一个 10–16K 聚合量级的 KV 池同时驻留在 8 卡上；当前形态
   超 3.3 GiB，做满两个 attention 杠杆后余量也只有 1.13 GiB。
2. **8 卡在当前驻留形态下（BF16 attention + DP 复制）跑不了任何档位**——
   连权重都装不下。

**不成立、已撤回的推论**：

3. ~~M4 的家不在 8 卡~~ / ~~单机口径全部失去形态基础~~ ——**这两条越过了
   证据边界**。M4 是 B=1–8、KV 极小的档位，其容量需求与 M7 相差一个数量级；
   本实验没有测过它。同理，"单机 128K 上下文""单机 ≥150 单路""单机 prefill
   ≥15,000"各有各的 KV/workspace 画像，**不能由 M7 的证伪推出**。

**正确的提法**（见 TARGET §7.7）：客户口径的标准版五行**不要求同时满足**，
每行可用各自的配置/拓扑。所以 8 卡不是一个"行不行"的问题，而是**五个独立的
容量问题**，每行需要各自的冻结配置（B / max_seq / KV dtype / 分片程度）、
可行性与余量。本实验提供了这五次计算的**输入**（第 3 节的逐层驻留字节），
但一个都没有回答。

4. **attention TP4 分片升级为"标准版五行的共同前提"**：第 4 节的 1.13 GiB
   余量本身就来自它；而 E2F 又表明单路 ≥150 也必须靠它。**这是当前最高价值的
   单项形态改动**，且它落地之前，五行里没有任何一行能被有意义地重问。

## 6. 未了结

0. **标准版五行的逐行容量：一个都没算**（TARGET §7.7）。本实验只回答了
   "当前形态 + 吞吐档位"这一种组合。
1. 只测了 decode 侧驻留。prefill 的 workspace 峰值更高，8 卡即便装下权重
   也未必跑得动 prefill——未测。
2. attention TP4 分片的**实现代价**（每层多一次 allreduce、KV 按 head 摆放、
   新图族、质量门）未评估。第 4 节的表只是字节账，不是可行性结论。
3. 未探索其他 8 卡形态（TP8×PP1 等）。TP8 被 TARGET §4.1 以 attention
   头数 padding 为由否决，但**该否决前提是 attention 被 TP 分片**；
   在 DP-attention 下 §4.1 的推导不直接适用。若要复活 8 卡，这条值得重看。

## 7. Artifact

复用 E2F 的探针与驱动，无新代码：

| 路径 | 内容 |
|---|---|
| `../../runtime/e2f_decode_phase_probe.py` | 探针（`--layers` 决定 stage 层数） |
| `../../runtime/run_e2f_probe.sh` | 驱动 |
| `../E2F-decode-latency-profile/results/out-e2f-pp2stage0/` | 22 层 OOM（rank JSON 里带 traceback） |
| `../E2F-decode-latency-profile/results/out-e2f-pp2max18/` | 18 层跑通 + 内存与 replay |
| `../E2F-decode-latency-profile/results/logs/e2f-pp2stage0-titan064.log` | OOM 原文与前后 nvidia-smi |
| `../E2F-decode-latency-profile/results/out-e2f-stage0-t064/` | 11 层对照 |

---

## 分片落地后的复测(2026-07-21 夜,执行 TARGET §7.7 第一行)

E6F 的 attention TP4 分片已实现并实测,于是本文第 4 节的字节账**可以验证了**。
全部在 titan064 GPU0-3、B=1、max_seq_len 3328、每次开跑前实测显存 1 MiB。

### 结果:仍差**恰好一层**

| 形态 | 单个 4 卡 TP4 stage 的最大层数 |
|---|---|
| 未分片(E3F 原测) | 18 |
| **+ attention 分片** | **21** |
| + 分片 + FP8 KV | **21**(无变化) |
| PP2 所需 | **22** |

分片值 **+3 层**;18 层处实测 free 由 **1.40 → 3.95 GiB(+2.55 GiB)**——
比 11 层 PP4 stage 上的 +1.42 GiB 大,因为节省随层数增长。

### 字节账偏乐观(**两次**,方向一致)

本文第 4 节预测"分片后 23.02 GiB、余 0.50"→ **实测 22 层 OOM**。
加上 E6F step 8 我自己的 prefill 预测在量级与方向上双双错误,
**本栈上的纸面账已连续两次偏乐观**。凡是余量敏感的判断,先测再写。

由 18 层实测反推(**推导值,非实测**):~1.087 GiB/层 →
21 层 ≈ 22.83(余 +0.69)、22 层 ≈ 23.92(**缺 0.40 GiB/卡**)。

### FP8 KV 无用,但**不是** flag 失效

22 层两次 OOM 数字逐字节相同(22.70 GiB 已分配、21.94 MiB 余),
这既像"杠杆用错地方",也像本竖条已犯过四次的**静默 no-op**。
按 §9.11 做了判别(18 层,能载入完):

| KV dtype | 18 层 free |
|---|---|
| bf16 | 3.95 GiB |
| fp8 | **3.96 GiB** |

**flag 生效**(有可测差异),但只值 **~10 MiB**:max_seq_len 3328 下 KV 已被
压缩比 4/128 压到极小,换半精度省不出东西。OOM 发生在
`build_physical_layer_material`(**权重加载**,KV 尚未分配),所以 KV dtype
本就打不到这个缺口。

### §7.7 第一行的答复

**"43 层模型能否装进 8 卡"= 目前不能,差 ~0.40 GiB/卡。**
未测的候选杠杆是 **FP8 常驻 attention 权重**(TARGET 估 +0.91 GiB/卡,
足以覆盖),它**尚未实现、也没有自己的质量门**。

⚠️ 装得下只回答第一行。E3F 的原始发现是**吞吐档位的 KV 池**装不下;
标准版 2–5 行(聚合吞吐、50 路并发、文档解析)各自仍需冻结配置与余量。
⚠️ 本节任何 tok/s 都**不是**标准版 headline(§9.13):22 层 4 卡是容量探针,
不是跑满 43 层的 8 卡系统。
