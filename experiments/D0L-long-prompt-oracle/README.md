...
## 扩展 golden（2026-07-21）：8192 档解锁，512 → 640 个比较位

**动机**：E6F 的 attention 分片 D0L 得 491/512（基线 494），按 §1.3 是 FAIL；
而 §1.3 自己说 ±3 可能只是这组 prompt 的偶然。**用 8 条 prompt 回答不了
"8 条 prompt 够不够"**，所以先把 golden 扩了。

**原来卡在哪**（TARGET §7.3）：`hc_post` 里
`comb.unsqueeze(-1) * residual.unsqueeze(-2)` 广播成 `[b,s,hc,hc,d]` fp32
= 256 KiB/token，8192 需要**恰好 2.00 GiB 的单次分配**，MP=8 下装不下——
所以 prompt 集里本来就有的两条 8192 一直没能生成。

**解法**：沿 s 分块。它**是精确的、不是近似**：唯一的归约在 dim=2（hc），
每个输出行只依赖它自己那个 token，**s 轴从不被求和**——切一个从不归约的轴
不可能改变任何值。
...
| 级别 | 结果 |
|---|---|
| 合成张量（`hc_post_chunk_selfcheck.py`） | **15/15 逐位**，s ∈ {128, **1000**, 1024, 2048, 4096} × chunk ∈ {256, 1024, 4096}；1000 是特意放的非整除长度 |
| s=8192 峰值 | 分块 1.688 GiB vs 不分块 3.688 —— **正好省 2.000 GiB**，与预测一致 |
| **真实模型（本次生成）** | **原有 8 条 prompt 的 prompt_tokens 与 completion_tokens 全部逐位一致** |

最后一行是关键：它把"分块是精确的"从合成张量上的论证，变成了**在 MP=8 真实
权重上的实测**。原有 golden 因此**没有任何一位改变**，冻结的 494 参照仍然有效。
...
|---|---:|---:|
| prompt 数 | 8 | **10** |
| 长度分布 | 1024×3 / 2048×3 / 4096×2 | + **8192×2** |
| 比较位 | 512 | **640** |
| 8192 档峰值 | —（OOM） | 22.72 GiB，**余 0.02 GiB** |

⚠️ **两点要说清楚**：
1. 8192 档余量只有 **0.02 GiB**——再长的 prompt 在当前 MP=8 形态下仍然不行，
   别把"8192 通了"读成"上限解除了"。
2. **这次扩展本身并不回答"494 是不是噪声"**。新增的 2 条来自同一语料、
   同一采样，它提高的是分辨率与长度覆盖（8192 是全新的 regime），
   而**真正的独立性检验需要另抽一组 prompt**。E6F 重新过门要以此为前提理解。

**下一步**：以 `oracle-long-v2.json` 重跑未分片基线，确立 640 位的新参照，
再用同一参照重测 E6F。
...
## ~~v2 扩展在 16 卡 runtime 上不可判~~ —— **本节结论已被自己推翻,见下方"更正"**

> **更正(同一晚,受控复测)**:本节的机制归因和结论**都是错的**。
> `max_seq_len` 在载入期**零成本**(4224 与 8320 实测 stage 0 均为 4.23 GiB
> free);真正的驱动量是 **MoE prefill 缓冲,按 prompt 长度派生的 row shape
> 逐个预分配**(`e0ef2e_golden_gate.py:938` `global_row_shapes`)。8192 那条
> 引入 32768 行档,单这一档约 **3.06 GiB**。
> 因此 `--prefill-chunk 4096` 直接解除限制:**v2 全部 10 条在 8320 下跑通,
> 614/640**,前 8 条合计 **494/512,与冻结基线逐字相同**。
> **扩展是可判的**,下方"不可判"的论述保留作为错误记录,勿再据此推导。

<details><summary>原始(错误)记述,保留备查</summary>


920313f 把 golden 从 8 prompt / 512 位扩到 10 prompt / 640 位,并记下计划:
"以 oracle-long-v2 重跑未分片基线,确立 640 位的新参照,再用同一参照重测
E6F"。**该计划不可行**,原因是实测的容量,不是实现问题。

### 事实

v2 = v1 的 8 条(逐字节相同,已校验)+ 2 条 8192。即扩展新增的内容
**恰好就是装不下的那部分**:

| oracle | 条数 | prompt 长度 |
|---|---|---|
| v1 | 8 | 3×1024, 3×2048, 2×4096 |
| v2 | 10 | v1 的 8 条 + **2×8192** |

`max_seq_len` 决定常驻 KV/索引缓存的分配,与当前 prompt 实际长度无关,
因此跑 8192 必须把 `max_seq_len` 从 4224 抬到 8320,而 stage 0 装不下:

| 运行 | max_seq_len | stage 0 载入后 free |
|---|---|---|
| 未分片 | 4224 | 4.23 GiB |
| **E6F 分片** | 4224 | **5.65 GiB** |
| 未分片 + `--embed-device cpu` | 8320 | 0.21 GiB |

换算(扣掉 CPU embed 让出的 0.986 GiB):8320 在未分片 + GPU embed 下等效
free 为 **−0.78 GiB**,即根本载入不了。

**+4096 位 = stage 0 常驻 +5.01 GiB**,即 1.252 MiB/位(11 层),
116.5 KiB/位/层。这一项此前被我按"latent 512 维 × 2 字节"估成 <100 MiB
并写作"可忽略",**是错的**,实测它是主导项;上面是实测值,机制拆分未知。

### 三个内存修复没有救回这一档

按 OOM 现场逐个修(每个都逐位验证,都保留):

| 修复 | 让出 | 结果 |
|---|---|---|
| `--embed-device cpu`(a16cbd3) | 0.986 GiB | 过了载入,prefill OOM |
| fp4 常量标量化(da4cd69) | ~0.22 GiB 瞬时 | 过了 indexer,`hc_post` OOM |
| `hc_post` 沿 s 分块(7e1673f) | 失败点 128→64 MiB | 仍 OOM,同一处 |

第三次 OOM 时 stage 0 **常驻已 22.61 GiB、仅余 25.88 MiB**——瓶颈已不是
瞬时量,削 workspace 不可能收敛到 5.01 GiB 的缺口。停止逐点修补。

### 推论(注意这条对 E6F 是坏消息而非好消息)

- 门在本拓扑上的上限是 **512 位 / max_seq_len 4224**;v2 的前 8 条就是 v1,
  所以**扩展对门的分辨率零增益**。
- 因此 **E6F 的 494 vs 491(差 3 位)是否噪声,用 v2 无法回答**。要回答它
  需要的是**独立重抽的 ≤4096 prompt 集**,不是更长的 prompt——这与扩展前
  的判断一致,扩展并未改变它。
- 算术上分片在 8320 下**反而装得下**(5.65 GiB 可用 vs 5.01 GiB 需要,余
  ~0.64 GiB)。这是**从两点外推的预测,不是实测**(本竖条已有一次预测在
  量级和方向上双双错误的前科,见 E6F step 8)。即便预测成立也**不能**用来
  比较:基线臂跑不起来的档位上,两臂不可比,任何"分片在 8192 下可用"的
  说法都缺少对照。
