...
用一个已知对象校准这条推论：ratio-4 层比 ratio-128 层多 **68 个核**
（291 vs 223，实测层段步长），而 ratio-4 独有的就是 indexer 链。
细相位表给同一件事的独立读数：`index_query_done` 91.0 µs/层，减去
`index_wq_b` 的 16.777 MB ÷ 806 GB/s = 20.8 µs，**余 ~66 µs 非 GEMV**
——与 68 核 × ~1 µs 吻合。
...
C4F 已为这条链写了 kernel/层级双逐位过门的融合核；能否直接用于 decode
需另测（C4F 的 90.5× 是 8192 行 prefill 口径，B=1 下收益机制不同：
不是省带宽，是省 68 次 kernel 启动）。
