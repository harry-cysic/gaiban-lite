...
- **prefill 实测(各 3 轮,轮间 ≤0.26%)**:torch attention 臂 **17,022**、
  ...
  **2,322**。归因:MoE 33.4%(其中 17.7 ms/层是 P2P 集合下界)、attention
  23.2%。(该臂已被下方 C4F 的 **28,622** 取代;C4F 的分相复核给出的同口径
  桶秒数为 ratio-4 0.3611 s = instrumented total 的 27.8%。)
...
- **增量 chunked prefill 已实现**([`C3F`](experiments/C3F-chunked-prefill/README.md)
  ...
  **吞吐上分段不划算**(整段 25,307 vs 分段 0.958–0.990×,单调);C3F 观察到的
  1.5–2.3× 已被同口径**证伪**(证伪本身是实测),真实规律是**规模经济**(每 token
  成本随单次 forward 行数下降),O(s²) 上下文税仅 +1.08%~+3.33%。
  ⚠️ C3F 与本次相反读数的**成因仍是推断而非实测**(疑为整段臂在 0.6 GiB 余量下
  的分配器压力):坐实需在 C3F 的 E2E 口径下补一个"整段 + 只注册 1 种形状 + 充足
  free"的对照臂,尚未跑——鉴于第二十二竖条已被同形状的分配器假说坑过一次,该假说
  在补测前不得当作已知。**分段的价值是容量**:峰值 20.409→14.095 GiB,且
...
- **ratio-4 attention 分相后拿到 +13.1%,且是零语义变更**
  ([`C4F`](experiments/C4F-ratio4-attention/README.md)):层内 30 相位 CUDA-event
  分相(插桩开销 0.13%,覆盖 100.02%,×5 层折算 0.3604 s 对 C2F 桶 0.3611 s
  差 0.2%)给出的头号项**不是 attention 本身**,而是 indexer query 的 QAT 仿真链
  `fp4_quant_dequant(hadamard_transform(·))`——**29.75 ms/层 = 41.3%,比 tilelang
  稀疏核(15.77 ms)还大**,且一个 GEMM 都没有:每步物化一个 268 MB 的 fp32 中间量,
  ~24 GB 往返对 0.268 GB 的信息量。融成一个 Triton kernel 后 **29.767 → 0.329 ms
  (90.5×,816 GB/s 已达搬运下界)**,**逐位相等**(5 组形状 0 个不等元素;真实权重
  层级 A/B 输出与三个状态张量全等)。C2F 同口径 3 轮 **28,609/28,622/28,657**,
  中位 **28,622 input tok/s = 1.131×**;ratio-4 桶 0.3611→0.2138 s 解释了整 pass
  −0.150 s 的 98.2%,其余三桶不动;**长门 494/512、逐 prompt 与 8 条
  `first_mismatch` 全字段与基线逐位相同**;每卡峰值 **20.409→17.845 GiB**。
  单池 T **2,322 → 2,538**。同轮判死三项:**wo_a 的 o_groups=8 分组 einsum 没有
  退化**(3.243 vs 手写 bmm 3.233 ms,块对角形式反而 8.1× 慢)、**投影 GEMM
  已在 BF16 张量峰值的 96–100%**(158–171 TFLOPS,prefill 无带宽余量可回收)、
  compressor fp32 cast 提取虽逐位相等但只值 0.07%(低于 0.167% 轮间噪声)。
  新头号项已定界:**tilelang 稀疏核 31% MFU + 包装层 16.1% 搬运税**
  (sm89 共享内存逼出的 head_chunk=16 循环)。
- 下一步:tilelang 稀疏核(31% MFU / 16.1% 包装税)、prefill HC 融合的正确实现
  (现占 27.2%,+19.9% 仍待取)、chunked prefill 的自有 golden(使其可默认开启)、
  P/D 同池实现与 serving。
