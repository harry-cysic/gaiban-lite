> **归档说明（2026-07-21）**：本文第 5 节的性能预估带（decode 15–25k、prefill
> 30–40k 等）已被实测证伪并由 `TARGET-v4-flash.md` 取代——目标与验收标准以该文为准。
> 本文保留为：① 实测修订记录（§5.5/§6）；② 运行环境清点（附录 B）；③ 容量与几何
> 推导的历史底稿。被证伪条目的清单见 `TARGET-v4-flash.md` §5.1。
...
- **prefill**：Pro 实测 ~700 in-tok/s/卡（⚠️ per-card,按 §9.13 不作 headline;此处仅作推导输入）× 激活比 3.8 × 注意力折扣（h 减半、topk 减半、
...
**chunked prefill 与 P/D 同池的实测修订（2026-07-21）**：增量 chunked prefill 已实现并过语义门（compressor 状态机逐位 27/27、topk 索引集精确 18/18；层级非逐位已归因到纯 GEMM 形状效应）。**吞吐上分段不划算**：C2F 同口径实测，整段 8192 = 25,307 tok/s（与冻结 25,308 差 0.004%），分段 2/4/8 段分别为 0.990/0.970/0.958×，单调递减且可复现。原因是**规模经济而非超线性罚金**：每 token 成本只取决于单次 forward 的行数并随其下降（1024→8192 行：161.2→158.1 µs/token/lane），分段等于落到更差的成本档；真正的 O(s²) 上下文税只有 +1.08%（2 段）到 +3.33%（16 段/16K）。**分段的价值是容量**：整段 8192 峰值 20.409 GiB → 8×1024 为 14.095 GiB（其中 MoE per-shape buffer −2.188 GiB = 80.03 KiB/行严格线性，activation −4.125 GiB，与实测峰值差 1 MiB 内闭合）；**整段 16384 在 4 卡全部 OOM，而 16×1024 只用 14.382 GiB**，线性拟合给出整段路径的上限 ≈11.4K token。即 chunking 把 prefill 峰值从 O(总长) 变成 O(段长)，这是它不可替代的性质。

**§6 "2 节点无 P/D 分离空间"需修正为有条件成立**：decode 的 8K 前沿（bl72）每卡余 0.531 GiB；整段 prefill 在共享权重之外还需 7.54 GiB（14×，故对整段 prefill 该判断成立），但 1024 段长只需 1.229 GiB，虽仍差 2.3× 却可用约 10 个 bl 的 decode 退让买到（bl72→约 bl62，约 −8% decode）。即结论从"结构上不可能"变为"**约 8% decode 代价 + 必须分段 prefill**"。三条限制：该评估仅基于显存（一个 1024 段会让 stage 阻塞约 165 ms）、decode 前沿是 fp8 KV 而 prefill 侧为 bf16 KV、这 1.229 GiB 必须从 KV 让出而非 graph pool。
...
| 2 节点无 P/D 分离空间，prefill 挤占 decode | 低中 | chunked prefill 交错（C2g 已验证）；必要时按 4K/1K 运营；**2026-07-21 修正**：该风险对整段 prefill 成立（需 7.54 GiB vs 余量 0.531），但分段 prefill（1024）只需 1.229 GiB，可用 ~8% decode 退让换取同池，详见 §5.5。 |
