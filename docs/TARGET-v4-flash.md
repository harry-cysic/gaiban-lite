...
模型级长 prompt golden 门（D0L），当前基线 **494/512**。任何性能改动若使该分数
下降即不放行，**容差不得放宽**。
...
**空白格优先级**：
1. **M4 延迟模式**（差 8–14×，且是唯一 0 投入的方向；第一步是 profile 见 §7.1）
2. **8 卡形态验证**——同时卡住 M4、M7 与单机口径的所有计划值（§7.2）
3. **M5 长上下文**（双机方案核心卖点，零覆盖）
4. M2/M3 继续推向计划带中上沿（M2 停在下沿则 serving 折扣后卡对外承诺边缘）
