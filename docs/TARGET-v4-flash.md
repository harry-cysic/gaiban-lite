...
| 单路 decode | **200–350**（+MTP 300–600） | 27.5（+MTP ~38） | **8–14%** | ❌ **从未针对性优化** |
...
| **M4 延迟 / 单用户** | **8 卡 TP4×PP2** | B=1–8 | **200–350**（+MTP 300–600） | 27.5（16 卡口径） | ❌ 8–14%，**未建** |
...
**空白格优先级（2026-07-21 定，理由见下）**：

1. **M4 profile**（不是优化）——一小时量级，**仪表清单须点名 indexer QAT 链**
   （C4F 的融合核已 kernel/层级双逐位，若该链在 B=1 显著则现成可用；先验估算它
   只值 0.2–0.4%，正因如此更要用 profile 定夺而不是直接启用）。见 §7.1。
2. **8 卡形态验证**——一次验证解锁三条线（M4 家园、M7、单机口径全部计划值）。见 §7.2。
3. 视 profile 结果决定 **M4 优化竖条** / **M5 长上下文**。
4. M2/M3 推向计划带中上沿（M2 停在下沿则 serving 折扣后卡对外承诺边缘）。
