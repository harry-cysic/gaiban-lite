...
语义链条完整且每一环都有真实权重的门背书。**本轮之后最大的风险不再是"8 卡没跑过"
——8 卡已跑过且装不下**（E3F）：M7 证伪，M4 回 16 卡，单机口径的全部计划值与
"单机聚合 ≥2,000"的书面承诺失去形态基础。而 M4 的 200–350 需要两处形态改动
（attention TP4 分片 + elementwise 尾巴折叠）同时到位，任一单独完成都停在
~80 tok/s（E2F）。**attention TP4 分片同时是 M4、8 卡形态与单机口径的共同前提，
是当前最高价值的单项形态改动。**
