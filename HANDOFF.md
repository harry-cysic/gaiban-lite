# HANDOFF（2026-07-22，goal session 更新）
...
| **E6F** | attention TP4 分片（A 档）：decode **+34.5%**、prefill **+13.6%**、每卡省 1.547 GiB | 实现完成、性能已测；**质量疑点已了结（噪声）**；**§1.3 字面仍 FAIL、默认关闭** |
| **D0L 扩展 + indep** | golden 上限解除；v2 640 位、indep 927 位（独立重抽） | 已落盘 |
| **§7.7 行 1** | 分片后 8 卡单 stage 上限 **21 层**（PP2 需 22），缺 ~0.40 GiB/卡 | 已落盘 |

**E6F 的质量结论（最重要的一条）**：两组独立 golden 一致判定
...
3. **E6F 放行与否**：需人裁定是否重新标定 §1.3 基准（见 §1.3 与 TARGET §2 优先级 3 step 10）。
